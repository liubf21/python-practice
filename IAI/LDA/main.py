...
def update_params_for_one_item(K, n_word_types, word_indices, counts, gammas, phi_d, log_betas, alpha, max_iter_d,
    ...
    while i < max_iter_d and delta >= tol_d:
        ...
        while n < n_word_types:
            ...
            # Update phi_d
            for k in range(K):
                phi_d[n, k] = np.exp(psi_gammas[k] + log_betas[w, k])
            phi_d[n, :] /= np.sum(phi_d[n, :])

            # Update gamma
            for k in range(K):
                gammas[k] += phi_d[n, k] * c

            # Update psi_gammas for the next iteration
            psi_gammas = psi(gammas)
...
def compute_bound_for_one_item(K, n_word_types, word_indices, count_vector, alpha, gammas, psi_gammas, phi_d,
    ...
    # E[log p(theta | alpha)]
    bound += gammaln(np.sum(alpha)) - np.sum(gammaln(alpha)) + np.sum((alpha - 1) * (psi_gammas - psi(np.sum(gammas))))

    # Entropy term for theta
    bound -= gammaln(np.sum(gammas)) - np.sum(gammaln(gammas)) + np.sum((gammas - 1) * (psi_gammas - psi(np.sum(gammas))))

    # E[log p(z | theta)]
    for n in range(n_word_types):
        for k in range(K):
            bound += count_vector[n] * phi_d[n, k] * psi_gammas[k]

    # E[log p(w | z, beta)]
    for n in range(n_word_types):
        word_index = word_indices[n]
        for k in range(K):
            bound += count_vector[n] * phi_d[n, k] * log_betas[word_index, k]

    # Entropy term for phi
    for n in range(n_word_types):
        for k in range(K):
            bound -= count_vector[n] * phi_d[n, k] * np.log(phi_d[n, k])
...
if __name__ == "__main__":
    ...
    # docs, _, vocab = preprocess("./dataset/dataset_cn.txt")
